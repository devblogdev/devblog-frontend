...
const PostEditor4 = (props) => {
  ...
  const saveDraft = (event) => {
    ...
    const rawPostData = {
      body: data,
      title: postExtraction[0]?.slice(1),
      abstract: postExtraction[1]?.slice(1),
      status: "draft",
    };
  ...
  const updateDraft = () => {
    ...
    const currentPost = props.user.posts.find(
  ...
  const deleteButton = (
    <DangerButton onClick={removePost} disableElevation>
      Delete
    </DangerButton>
  );
  ...
  const loadedDraftOrPost = useMemo(
    () =>
      props.user.posts?.find(
        (post) => `${post.id}` === props.match.params.postID
      ),
    [props.user, props.match]
  );
  ...
  const loadedInitialEditorState = useMemo(
    () => EditorState.createWithContent(convertFromHTML(dummyDraftOrPost.body)),
    [dummyDraftOrPost.body]
  );
  ...
  useEffect(() => {
    if (props.match.url !== "/profile/drafts/new") {
      if (props.user.posts) {
        ...
        dispatch(
          registerDraftOrPostBodyImages(convertToRaw(contentState), {
            type: "initial",
          })
        );
      }
  ...
  function uploadImageCallback(file) {
    return new Promise((resolve, reject) => {
      ...
      const coverImageAlbum = "UtZ3xQY";
      formData.append("image", file);
      formData.append("album", coverImageAlbum);
      axios
  ...
  return (
    <div className="App postEditor">
      ...
      <header className="posteditor-header">Post Editor</header>
